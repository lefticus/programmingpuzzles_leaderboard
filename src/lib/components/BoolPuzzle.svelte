<script lang="ts">
	...
	const varLine = $derived(round?.promptLabel ?? '');
	...
	function handleInput() {
		// Only allow 0, 1, T, F (case-insensitive), single character
		userAnswer = userAnswer.replace(/[^01tTfF]/g, '');
	...
	function handleSubmit(e: Event) {
		...
		// Normalize: T/t → 1, F/f → 0
		const raw = userAnswer.trim().toUpperCase();
		const normalized = raw === 'T' ? '1' : raw === 'F' ? '0' : raw;
...
<div class="conversion-puzzle">
	...
	<div class="prompt-area">
		<div class="var-line mono">{varLine}</div>
